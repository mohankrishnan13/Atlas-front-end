...
export async function GET(request: Request) {
  try {
    ...
    const response = await fetch(`${API_URL}/api/v1/dashboard/db-monitoring?env=${env}`, {
      headers,
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error.detail || 'Failed to fetch DB monitoring data' },
    ...
    console.error('DB monitoring API error:', error);
    return NextResponse.json(
